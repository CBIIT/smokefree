 ...
 */
function smokefreegov_form_element_label(&$variables) {
  ...
  // Render Yes/No inputs differently for the SFVet Quiz.
  $path = current_path();
  // @todo Will we be expanding this styling to other parts of the site?
  // If so, we could apply this style to all yes/no radios by removing the
  // first if statement.
  if ($path == 'node/149') {
    $title_value = trim(strtolower($title));
    if ($radio && in_array($title_value, array('yes', 'no'))) {
      $yes_no_radio = TRUE;
      $display = 'outside';
    }
  }
  ...
  // Provide option to display the option tag outside of the label tag.
  elseif ($display === 'outside') {
    if (isset($yes_no_radio)) {
      $title_value = trim(strtolower($title));
      $attributes['class'][] = "form-radio-label-$title_value";
    }
    return $element['#children'] . ' <label' . drupal_attributes($attributes) . '>' . $title . "</label>\n";
  }
  else {
